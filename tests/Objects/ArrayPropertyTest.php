...
class ArrayPropertyTest extends AbstractPHPModelGeneratorTestCase
{
    ...
    public function testInvalidObjectArrayThrowsAnException(
        ...
        try {
            ...
            // (a no-op for the non-combined nested/referenced cases, which have no such class).
                ...
                [
                    'invalid type bool' => [
                        ...
                        function (InvalidItemException $exception): void {
                            ...
                            // Re-routing makes the item an object property, so a scalar item fails
                            // the object type check directly instead of an allOf composition.
                            $itemException = $exception->getInvalidItems()[1][0];
                            Assert::assertInstanceOf(InvalidTypeException::class, $itemException);
                            Assert::assertSame(true, $itemException->getProvidedValue());
                            Assert::assertSame(
                                '/property/1',
                                $itemException->getInstancePointer()->pointer,
                            );
                        },
                    ],
                    'missing property name' => [
