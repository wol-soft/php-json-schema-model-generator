...
class PropertyProxy extends AbstractProperty
{
     ...
     * @inheritdoc
     *
     * Filters both the shared underlying property's attributes and the proxy's own locally
     * added ones. Filtering only the underlying would leave locally added attributes (e.g. from
     * repeated composition attribute synthesis) unfiltered, so a filter-then-add cycle called
     * more than once on the same proxy would accumulate duplicates instead of replacing them.
     */
